...
function AllowedUsersSection() {
  ...
  function ini(s: string) { return s.substring(0, 2).toUpperCase() }

  async function loadUsers() {
    setLoading(true)
    const u = await fetchAllowedUsers()
    setUsers(u)
    setLoading(false)
  }
  ...
  // ── Add flow ───────────────────────────────────────────────────
  function openAdd() { setAddEmail(''); setAddLabel(''); setAddError(''); setAddStep('form'); setShowAdd(true) }
  function closeAdd() { setShowAdd(false); setAddError('') }

  function handleAddNext() {
    if (!addEmail.trim() || !addEmail.includes('@')) { setAddError('Enter a valid email address.'); return }
    if (users.some(u => u.email.toLowerCase() === addEmail.trim().toLowerCase())) {
      setAddError('This email is already on the allowlist.'); return
    }
    setAddError('')
    setAddStep('confirm')
  }
  ...
  async function handleAddConfirm() {
    ...
    const err = await addAllowedUser(addEmail.trim(), addLabel.trim() || undefined)
    if (err) { setAddError(err); setSaving(false); return }
    await loadUsers()
    setSaving(false)
    closeAdd()
  }
  ...
  async function handleRemoveConfirm() {
    ...
    await removeAllowedUser(removeTarget)
    setUsers(u => u.filter(x => x.email !== removeTarget))
    setRemoving(null)
  ...
  return (
    <>
      {/* Row button — same style as other settings rows */}
      <Row
        icon="🔐"
        label="Manage allowed users"
        sub={`Control who can sign in to INFolio`}
        onClick={() => setOpen(true)}
      >
        <svg width="14" height="14" viewBox="0 0 14 14" className="text-textmut shrink-0">
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          style={{ background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)' }}>
          <div className="bg-white rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.18)] w-full max-w-md flex flex-col max-h-[85vh]">

            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#F0EEE9]">
              <div>
                <h3 className="text-[15px] font-black text-[#1A1A1A]">Access Control</h3>
                <p className="text-[11px] text-[#767676] mt-0.5">Only these emails can sign in</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4">
              {loading ? (
                <div className="flex items-center gap-2 text-[12px] text-[#767676] py-4">
                  <span className="w-3.5 h-3.5 rounded-full border-2 border-[#0F766E] border-t-transparent animate-spin inline-block" />
                  Loading…
                </div>
              ) : (
                      ...
                      <div key={u.email} className="flex items-center gap-3 rounded-xl bg-[#F5F4F0] border border-[#E0DDD6] px-3 py-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="text-[13px] font-semibold text-[#1A1A1A] truncate">{u.email}</span>
                            {isYou && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-[#0F766E]/10 text-[#0F766E]">you</span>}
                            {u.label && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-[#F0EEE9] text-[#767676]">{u.label}</span>}
                          </div>
                        ...
                        {!isYou && (
                          <button
                            ...
                            className="text-[11px] font-semibold text-[#C0392B] hover:bg-red-50 px-2 py-1 rounded-lg border border-red-200 transition-colors shrink-0 disabled:opacity-40"
                          >
                            Remove
                          </button>
                        )}
            ...
            {/* Footer */}
            <div className="px-5 py-4 border-t border-[#F0EEE9]">
              <button onClick={openAdd}
                className="w-full h-10 rounded-xl bg-[#0F766E] text-white text-[13px] font-bold hover:bg-[#0D4F4A] transition-all flex items-center justify-center gap-2">
                + Add user
              </button>
            </div>
          </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.2)] w-full max-w-sm p-6">

            {addStep === 'form' ? (
              <>
                <div className="flex items-center justify-between mb-5">
                  <h3 className="text-[15px] font-black text-[#1A1A1A]">Add user</h3>
                  <button onClick={closeAdd} className="text-[#ABABAB] hover:text-[#1A1A1A] text-lg leading-none">✕</button>
                </div>
                <div className="flex flex-col gap-3 mb-5">
                  <div>
                    <label className="text-[10px] font-bold uppercase tracking-widest text-[#767676] mb-1.5 block">Email address</label>
                    <input
                      type="email"
                      ...
                      autoFocus
                      className="w-full h-10 rounded-xl bg-[#F5F4F0] border border-[#E0DDD6] text-[13px] text-[#1A1A1A] placeholder:text-[#ABABAB] outline-none px-3.5 focus:border-[#0F766E] focus:ring-2 focus:ring-[#0F766E]/10 focus:bg-white transition-all"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold uppercase tracking-widest text-[#767676] mb-1.5 block">
                      Label <span className="normal-case font-normal text-[#ABABAB]">optional</span>
                    </label>
                    <input
                      type="text"
                      placeholder="e.g. Owner, Family, Friend"
                      value={addLabel}
                ...
                {addError && (
                  <div className="flex items-center gap-2 bg-red-50 border border-red-100 rounded-xl px-3 py-2 mb-4">
                    <span className="text-red-500 text-xs">⚠</span>
                    <p className="text-[12px] text-[#C0392B]">{addError}</p>
                  </div>
                )}
              ...
              <>
                <div className="flex items-center justify-between mb-5">
                  <h3 className="text-[15px] font-black text-[#1A1A1A]">Confirm access</h3>
                  <button onClick={() => setAddStep('form')} className="text-[#ABABAB] hover:text-[#1A1A1A] text-lg leading-none">✕</button>
                </div>
                <div className="bg-[#F5F4F0] rounded-xl p-4 mb-5">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 rounded-full bg-[#E0DDD6] flex items-center justify-center text-[12px] font-bold text-[#5F5E5A] shrink-0">
                      {ini(addEmail)}
                    </div>
                    <div>
                      <div className="text-[13px] font-bold text-[#1A1A1A]">{addEmail}</div>
                      {addLabel && <div className="text-[11px] text-[#767676]">{addLabel}</div>}
                    </div>
                  </div>
                  <p className="text-[12px] text-[#767676] leading-relaxed">
                    This person will be able to sign in to INFolio. They will only see their own data unless you grant them profile access separately.
                  </p>
                </div>
                {addError && (
                  <div className="flex items-center gap-2 bg-red-50 border border-red-100 rounded-xl px-3 py-2 mb-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={handleAddConfirm} disabled={saving}
      ...
      {removeTarget && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4"
          style={{ background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }}>
          <div className="bg-white rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.2)] w-full max-w-sm p-6">
            <div className="w-12 h-12 rounded-xl bg-red-50 flex items-center justify-center text-xl mb-4 mx-auto">🗑️</div>
            <h3 className="text-[15px] font-black text-[#1A1A1A] text-center mb-2">Remove user?</h3>
            <p className="text-[12px] text-[#767676] text-center mb-1">
              <span className="font-semibold text-[#1A1A1A]">{removeTarget}</span>
            </p>
            <p className="text-[12px] text-[#767676] text-center mb-5">
              They will no longer be able to sign in to INFolio.
            </p>
            <div className="flex gap-2">
              ...
              <button onClick={handleRemoveConfirm} disabled={!!removing}
                className="flex-1 h-10 rounded-xl bg-[#C0392B] hover:bg-[#A93226] text-white text-[13px] font-bold transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                {removing ? <span className="w-3.5 h-3.5 rounded-full border-2 border-white/30 border-t-white animate-spin inline-block" /> : null}
