...
function renderZerodhaActualInvested(rows) {
  ...
  body.innerHTML = rows.map((r, i) => {
    ...
    return `<tr style="background:${i % 2 === 0 ? '#fff' : 'var(--surface2)'}">
        <td class="zai-cb-wrap" style="width:28px;padding:0 8px;display:none;border-bottom:1px solid var(--border)"><input type="checkbox" class="zai-cb" data-id="${r.id}" style="width:14px;height:14px;cursor:pointer;accent-color:#0d9488"></td>
  ...
  if (window['_zerodha_bindCheckboxes']) window['_zerodha_bindCheckboxes']();
  body.querySelectorAll('.zai-edit-btn').forEach(btn => {
    btn.addEventListener('click', () => openZaiModal({
      id: btn.dataset.zaiId, entry_date: btn.dataset.zaiDate, amount: btn.dataset.zaiAmount
    }));
  });
}
...
async function importZerodhaStocks(allRows) {
  ...
  const { data: existing } = await sb.from('zerodha_stocks').select('instrument, qty').eq('user_id', _currentUserId);
  const prevQtyMap = {};
  ...
  const { error } = await sb.from('zerodha_stocks').upsert(
    rows.map(r => ({
...
let _editingZerodhaCurrentQty = null;

function openZerodhaEditModal(row) {
  _editingZerodhaId         = row.id;
  _editingZerodhaCurrentQty = +row.qty || 0;
...
document.addEventListener('fragments-loaded', () => {
  ...
  document.getElementById('zerodha-refresh-btn')?.addEventListener('click', () => {
    if (_currentAssetFilter === 'Zerodha Stocks') loadAssets(_currentUserId, 'Zerodha Stocks');
  });
  ...
  modal2?.addEventListener('click', e => { if (e.target === modal2) closeZerodhaImportModal(); });
  ...
  // ── Edit modal ──────────────────────────────────────────────
  const modal3 = document.getElementById('zerodha-edit-modal');
  ...
  modal3?.addEventListener('click', e => { if (e.target === modal3) closeZerodhaEditModal(); });
  ...
  document.getElementById('zerodha-edit-save-btn')?.addEventListener('click', async () => {
    ...
    if (qty !== _editingZerodhaCurrentQty) updatePayload.prev_qty = _editingZerodhaCurrentQty;

    const { error } = await sb.from('zerodha_stocks').update(updatePayload).eq('id', _editingZerodhaId);
  ...
  (function() {
    ...
    function _enter() {
      _sel = true;
      var btn = document.getElementById('zerodha-select-btn');
      ...
      document.getElementById('zerodha-bulk-bar')?.classList.remove('hidden');
      document.querySelectorAll('.zai-cb-wrap').forEach(c => { c.style.display = ''; });
      _upd();
    }
    ...
    function _exit() {
      ...
      document.getElementById('zerodha-bulk-bar')?.classList.add('hidden');
      document.getElementById('zerodha-bulk-normal').style.display = 'flex';
      document.getElementById('zerodha-bulk-confirm').style.display = 'none';
      document.querySelectorAll('.zai-cb-wrap').forEach(c => { c.style.display = 'none'; });
      document.querySelectorAll('.zai-cb').forEach(c => { c.checked = false; });
      _upd();
    }

    function _upd() {
      var n       = document.querySelectorAll('.zai-cb:checked').length;
      var countEl = document.getElementById('zerodha-bulk-count');
      var delBtn  = document.getElementById('zerodha-bulk-delete');
      if (countEl) countEl.textContent = n + ' selected';
      if (delBtn)  delBtn.disabled = n === 0;
    }

    document.getElementById('zerodha-select-btn')?.addEventListener('click', () => { if (_sel) _exit(); else _enter(); });
    document.getElementById('zerodha-bulk-cancel')?.addEventListener('click', _exit);

    document.getElementById('zerodha-bulk-delete')?.addEventListener('click', () => {
      var n = document.querySelectorAll('.zai-cb:checked').length;
      if (!n) return;
      document.getElementById('zerodha-bulk-normal').style.display = 'none';
      document.getElementById('zerodha-bulk-confirm').style.display = 'flex';
      document.getElementById('zerodha-bulk-confirm-count').textContent = n === 1 ? '1 entry' : n + ' entries';
    });

    document.getElementById('zerodha-bulk-no')?.addEventListener('click', () => {
      document.getElementById('zerodha-bulk-normal').style.display = 'flex';
      document.getElementById('zerodha-bulk-confirm').style.display = 'none';
    });

    document.getElementById('zerodha-bulk-yes')?.addEventListener('click', async () => {
      var checked = [...document.querySelectorAll('.zai-cb:checked')];
      if (!checked.length) return;
      var yesBtn = document.getElementById('zerodha-bulk-yes');
      yesBtn.textContent = 'Deleting…'; yesBtn.disabled = true;
      var anyErr = false;
      for (var cb of checked) {
        var r = await sb.from('zerodha_actual_invested').delete().eq('id', cb.dataset.id).then(r=>r);
    ...
    window['_zerodha_bindCheckboxes'] = function() {
      document.querySelectorAll('.zai-cb').forEach(cb => { cb.addEventListener('change', _upd); });
      document.querySelectorAll('.zai-cb-wrap').forEach(c => { c.style.display = _sel ? '' : 'none'; });
      if (_sel) _upd();
    };
  })();
