...
function renderZerodhaActualInvested(rows) {
  ...
  body.innerHTML = rows.map((r, i) => {
    ...
    return `<tr style="background:${i % 2 === 0 ? '#fff' : 'var(--surface2)'}">
        <td class="zai-cb-wrap" data-id="${r.id}" style="width:28px;padding:0 8px;display:none;border-bottom:1px solid var(--border)"><input type="checkbox" class="zai-cb" data-id="${r.id}" style="width:14px;height:14px;cursor:pointer;accent-color:#0d9488"></td>
...
(function() {
  ...
  function _enter() {
    ...
    document.querySelectorAll('.zai-cb-wrap').forEach(function(c) { c.style.display = ''; });
    _upd();
  }
  ...
  function _exit() {
    ...
    document.querySelectorAll('.zai-cb-wrap').forEach(function(c) { c.style.display = 'none'; });
    document.querySelectorAll('.zai-cb').forEach(function(c) { c.checked = false; });
    _upd();
  }
  ...
  document.addEventListener('fragments-loaded', function() {
    ...
    document.getElementById('zerodha-bulk-delete')?.addEventListener('click', function() {
      var n = document.querySelectorAll('.zai-cb:checked').length;
      if (!n) return;
    ...
    document.getElementById('zerodha-bulk-no')?.addEventListener('click', function() {
      document.getElementById('zerodha-bulk-normal').style.display = 'flex';
      document.getElementById('zerodha-bulk-confirm').style.display = 'none';
    });

    document.getElementById('zerodha-bulk-yes')?.addEventListener('click', async function() {
  ...
  window['_zerodha_bindCheckboxes'] = function() {
    document.querySelectorAll('.zai-cb').forEach(function(cb) {
      cb.addEventListener('change', _upd);
    });
    // Hide all checkbox cells by default unless in select mode
    document.querySelectorAll('.zai-cb-wrap').forEach(function(c) {
